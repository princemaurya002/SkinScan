...
def _frankot_chellappa_local(p, q):
    """Local Frankot-Chellappa integration (no wavepy dependency)."""
...
class Mesh:
    ...
    def setDepth(self):
        ...
        p = np.nan_to_num(p)
        q = np.nan_to_num(q)
        # Frankot-Chellappa integration: use wavepy if available, else local FFT implementation
        if _has_wavepy:
            try:
                x = wavepy.surface_from_grad.frankotchellappa(p, q, False)
                depth = x.real
            except Exception:
                depth = _frankot_chellappa_local(p, q)
        else:
            depth = _frankot_chellappa_local(p, q)
