...
class Webcam(External):
    """Default webcam (external camera on port 0). Use for pipeline with USB webcam."""
    def __init__(self, external_camera_port=0, **kwargs):
        super().__init__(external_camera_port=external_camera_port, **kwargs)


class PhoneCam(Camera, ABC):
    """
    Camera that reads from a phone via IP/URL (e.g. IP Webcam or DroidCam).
    Use your Motorola Edge 60 (or any Android) as the camera: install an app that
    streams video over WiFi, then set the URL in config or PHONE_CAM_URL.
    """
    def __init__(self, url, exposure=0.01, white_balance=0, auto_focus=True, grayscale=True):
        self.url = url
        self.cap = cv2.VideoCapture(url)
        if self.cap.isOpened():
            time.sleep(0.5)
        if self.cap is None or not self.cap.isOpened():
            print("Warning: unable to open phone camera at URL:", url)
        fps = self.getFPS()
        resolution = self.getResolution()
        self.grayscale = grayscale
        super().__init__(exposure, white_balance, auto_focus, fps, resolution, grayscale)
    ...
    def getImage(self, name='test', saveImage=True, saveNumpy=True, calibration=False, timeout=5000):
        ...
        if not ret or frame is None:
            raise RuntimeError("Phone camera did not return a frame. Check URL and that the app is streaming.")
        if saveImage:
            if calibration:
                cv2.imwrite('CalibrationImages/' + name + '.PNG', cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            else:
    ...
    def getStatus(self):
        if self.cap is None or not self.cap.isOpened():
            print('Warning: unable to open phone camera at URL:', self.url)
